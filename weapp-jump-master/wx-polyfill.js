(function () {
  ...
  wx.onTouchEnd = function (cb) {
    ...
    document.addEventListener('mouseup', function (e) {
      var t = { touches: [], changedTouches: [{ clientX: e.clientX, clientY: e.clientY }], timeStamp: e.timeStamp }
      cb(t)
    })
  }
  ...
  /* ======================== 振动 / 震感 ======================== */
  function __doVibrate(duration) {
    try {
      navigator.vibrate(duration);
      console.log('[vibrate] navigator.vibrate(' + duration + ') OK');
    } catch (e) {
      console.log('[vibrate] navigator.vibrate FAIL:', e.message);
      if (window.AndroidVibrator) {
        AndroidVibrator.vibrate(duration);
        console.log('[vibrate] AndroidVibrator fallback OK');
      }
    }
  }
  wx.vibrateShort = function (opts) {
    __doVibrate((opts && opts.duration) || 15);
    opts && opts.success && opts.success({ errMsg: 'vibrateShort:ok' });
    opts && opts.complete && opts.complete({ errMsg: 'vibrateShort:ok' });
  }
  wx.vibrateLong = function (opts) {
    __doVibrate(200);
    opts && opts.success && opts.success({ errMsg: 'vibrateLong:ok' });
    opts && opts.complete && opts.complete({ errMsg: 'vibrateLong:ok' });
  }
  window.__vibrateShort = function (d) { __doVibrate(d) }
  window.__vibrateLong = function () { __doVibrate(200) }

  /* startup self-test */
  setTimeout(function () {
    var info = 'navigator.vibrate' in window ? 'AVAILABLE' : 'UNAVAILABLE';
    console.log('[vibrate] navigator.vibrate API: ' + info);
    info = 'AndroidVibrator' in window ? 'AVAILABLE' : 'UNAVAILABLE';
    console.log('[vibrate] AndroidVibrator bridge: ' + info);
    if ('navigator' in window && 'vibrate' in navigator) {
      console.log('[vibrate] Self-test: brief vibration in 1s...');
      setTimeout(function () { navigator.vibrate(50); }, 1000);
    }
  }, 2000);
